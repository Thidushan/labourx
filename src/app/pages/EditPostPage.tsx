...
import { mockWorkPosts } from '../data/mockData';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center p-8">
          ...
          <h2 className="text-foreground mb-2" style={{ fontWeight: 700 }}>Post not found</h2>
  ...
  if (currentUser.id !== post.userId) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center p-8">
          <FileText className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
          <h2 className="text-foreground mb-2" style={{ fontWeight: 700 }}>Not Authorized</h2>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center p-8 max-w-md">
          ...
          <h2 className="text-foreground mb-3" style={{ fontSize: '1.5rem', fontWeight: 700 }}>Post Updated!</h2>
          <p className="text-muted-foreground mb-6">Your changes to "<span style={{ fontWeight: 600 }} className="text-foreground">{form.title}</span>" have been saved successfully.</p>
          <div className="flex gap-3 justify-center">
            <Link to={`/posts/${post.id}`} className="bg-maroon hover:bg-maroon-dark text-white px-5 py-2.5 rounded-xl text-sm transition-colors" style={{ fontWeight: 600 }}>View Post</Link>
            <Link to="/my-projects" className="border border-border text-foreground hover:bg-muted px-5 py-2.5 rounded-xl text-sm transition-colors" style={{ fontWeight: 500 }}>My Projects</Link>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-maroon py-8">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-white" style={{ fontSize: '1.75rem', fontWeight: 700 }}>Edit Job Post</h1>
          <p className="text-white/70 mt-1">Update the details of your project listing</p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <label className={`${labelClass} flex items-center gap-1`} style={{ fontWeight: 600 }}>
              Post Status
            </label>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className={labelClass} style={{ fontWeight: 500 }}>
                  <Tag className="w-3.5 h-3.5 inline mr-1 text-maroon" />Category *
                </label>
          ...
          <div className="bg-card border border-border rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={form.location} onChange={e => update('location', e.target.value)} className={inputClass} required />
              </div>
              <div>
          ...
          <div className="bg-card border border-border rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className={labelClass} style={{ fontWeight: 500 }}>Min Budget (Rs.) *</label>
                <input type="number" value={form.budgetMin} onChange={e => update('budgetMin', e.target.value)} className={inputClass} required min="0" />
              </div>
              <div>
                <label className={labelClass} style={{ fontWeight: 500 }}>Max Budget (Rs.) *</label>
                <input type="number" value={form.budgetMax} onChange={e => update('budgetMax', e.target.value)} className={inputClass} required min="0" />
              </div>
              <div>
                <label className={labelClass} style={{ fontWeight: 500 }}>
                  <Clock className="w-3.5 h-3.5 inline mr-1 text-maroon" />Timeline *
                </label>
                <input type="text" value={form.timeline} onChange={e => update('timeline', e.target.value)} className={inputClass} required />
              </div>
            </div>
          </div>
