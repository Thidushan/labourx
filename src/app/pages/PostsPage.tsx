...
export function PostsPage() {
  ...
  const filtered = mockWorkPosts.filter(post => {
    const matchSearch = !search || post.title.toLowerCase().includes(search.toLowerCase()) || post.description.toLowerCase().includes(search.toLowerCase()) || post.location.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          >
            All Posts ({mockWorkPosts.length})
          </button>
          {SPECIALTIES.slice(0, 6).map(s => {
            const count = mockWorkPosts.filter(p => p.category === s).length;
